    ...
    try {
        ...
        switch (ActionID) {
            case 3:
                ...
                sql += " ) ";
                sql += " INSERT INTO help_center(module_id,title,ques_content,asw_content,ques_uid,ip,asw_date,asw_uid,is_system)  ";
                sql += " SELECT   module_id,title,'系统帮助手册',asw_content,'"+login.getUserId()+"','"+login.getIpAddr()+"',getdate(),'"+login.getUserId()+"',1 FROM T1 ";
                break;
            ...
            case 4:
                ...
                while (e.hasMoreElements()) {
                    n = (String) e.nextElement();
                     if (!"Action".equals(n) && !"btn".equals(n) && !"id".equals(n) && !n.startsWith("ext-") && !n.startsWith("fck")) {
                        v = stringUtil.nullValue(request.getParameter(n), "");
                        sql += ("".equals(sql) ? "" : ",") + n + "='" + v + "' ";
                    }
                }
                sql = "UPDATE help_center SET " + sql;
                sql += " WHERE id='" + id + "' ";
                break;
            case 5:
                sql = " delete from  help_center  where  id IN (" + stringUtil.strComma2Singlequotes(id) + ")";
                break;
        }
